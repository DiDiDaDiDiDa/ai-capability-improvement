...
这张表是**声称**，代码能跑才是**事实**。上次就出现过表里写「已完成」但仓库无对应代码的情况。
复核命令（2026-08-06 全部实跑通过）：

```bash
# 三大项目验收：均应 EXIT=0
for p in p1-enterprise-rag p2-ai-gateway p3-mini-coding-agent; do
  (cd projects/$p && python3 app.py >/dev/null 2>&1; echo "$p EXIT=$?")
done

# 实验：17 个脚本全绿（mini-semantic-search 需下载 470MB 模型，单独手动跑）
