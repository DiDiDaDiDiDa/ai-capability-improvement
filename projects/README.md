# Projects · 实战项目（倒逼学习）

三个项目对应你的模块 6「工业级实战项目倒逼」。它们不是学完才做，而是与知识模块**并行推进**——用项目需求驱动补齐知识盲区。

| # | 项目 | 目录 | 时间 | 何时启动 | 串联模块 |
|---|------|------|------|----------|----------|
| P1 | 企业级 RAG 平台 | [`p1-enterprise-rag/`](p1-enterprise-rag/) | ~20h | 学到模块 03 | 01 / 02 / 03 / 06 |
| P2 | AI Gateway 增强版 | [`p2-ai-gateway/`](p2-ai-gateway/) | ~15h | 学到模块 06 | 02 / 06 |
| P3 | Mini Coding Agent | [`p3-mini-coding-agent/`](p3-mini-coding-agent/) | ~25h | 学完模块 04 | 02 / 03 / 04 / 06 |

验收标准：每个项目都能 `run` 起来、能演示核心链路。

## 进度速览

| 项目 | 状态 | 怎么跑 |
|------|------|--------|
| P1 企业级 RAG | ✅ M1–M4 | `cd p1-enterprise-rag && python3 app.py` |
| P2 AI Gateway | ⬜ 能力清单待实现；接缝已在 P1 `LLMProvider` 预留 | 见 `p2-ai-gateway/README.md` |
| P3 Mini Coding Agent | 🟡 M1+M2（轻量 M3） | `cd p3-mini-coding-agent && python3 app.py` |
